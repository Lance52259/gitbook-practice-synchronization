...
func TestDetectAttachedVolumeAlreadySyncedViaSourceLink(t *testing.T) {
	tmp := t.TempDir()
	b := filepath.Join(tmp, "b")
	c := filepath.Join(tmp, "c")
	_ = os.MkdirAll(filepath.Join(b, "examples", "ecs", "attached-volume"), 0o755)
	_ = os.WriteFile(filepath.Join(b, "examples", "ecs", "attached-volume", "main.tf"), []byte(""), 0o644)
	_ = os.MkdirAll(filepath.Join(c, "docs", "zh-cn", "best-practices", "ecs"), 0o755)
	_ = os.WriteFile(filepath.Join(c, "docs", "zh-cn", "best-practices", "ecs", "instance_with_volume.md"), []byte(`# 部署

## 参考信息

- [源码](https://github.com/huaweicloud/terraform-provider-huaweicloud/tree/master/examples/ecs/attached-volume)
`), 0o644)
	...
	if len(result.NewPractices) != 0 {
		t.Fatalf("attached-volume must not be new when instance_with_volume.md links to it, got %+v", result.NewPractices)
	}
}
